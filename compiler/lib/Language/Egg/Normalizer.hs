...
anf i (Tuple es l)      = (i', stitch bs (Tuple es' l))
  where
    (i', bs, es')       = imms i es
-- anf likely similar to app
--normalizer should be good for now
anf i (GetItem e1 e2 l) = (i'', stitch bs'' (GetItem e1' e2' l))
 where
    bs''                = bs' ++ bs
    (i' , bs , e1')     = imm i  e1
    (i'', bs', e2')     = imm i' e2
--simiplar to prim2 prob
--get item takes 2 expressions 
--------------------------------------------------------------------------------
...
--tuple was added in this iteration as well as getitem
...
--similar to prim2s normalize function
